...
public class Call extends MonitoredObject implements InviteDialogListener {
   ...
   {  if (dialog!=null) dialog.bye();
     ...
     */
    public void hangup() {
        if (dialog != null) {  // try dialog.refuse(), cancel(), and bye() methods..
            //dialog.refuse();
            //dialog.cancel();
            //dialog.bye();
            if (isIdle()) dialog.cancel();
            else if (isIncoming()) dialog.refuse();
            else if (isOutgoing()) dialog.cancel();
            else if (isActive()) dialog.bye();
            else if (isClosed()) dialog.bye();
    ...
    public void onDlgInvite(InviteDialog d, NameAddress callee, NameAddress caller, String sdp, Message msg) {
        if (d != dialog) {
            printLog("NOT the current dialog", Log.LEVEL_HIGH);
            return;
        }
        changeStatus(C_INCOMING);
