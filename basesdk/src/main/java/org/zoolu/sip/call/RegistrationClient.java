...
public class RegistrationClient implements TransactionClientListener, TimerListener {
    ...
    protected void register(int expire_time, String body) {
        ...
        if (expire_time > 0)
            printLog("registering contact " + contact + " (it expires in " + expire_time + " secs)", Log.LEVEL_HIGH);
   ...
   {  loopRegister(expire_time,renew_time);
    ...
    public void onTransFailureResponse(TransactionClient transaction, Message resp) {
        if (transaction.getTransactionMethod().equals(SipMethods.REGISTER)) {
            ...
            if (code == 401 && attempts < SipStack.regc_auth_attempts && resp.hasWwwAuthenticateHeader() && resp.getWwwAuthenticateHeader().getRealmParam().equalsIgnoreCase(realm)) {  // UAS authentication
                attempts++;
                ...
                TransactionClient t = new TransactionClient(sip_provider, req, this);
                t.request();
            } else if (code == 407 && attempts < SipStack.regc_auth_attempts && resp.hasProxyAuthenticateHeader() && resp.getProxyAuthenticateHeader().getRealmParam().equalsIgnoreCase(realm)) {  // Proxy authentication
                attempts++;
                Message req = transaction.getRequestMessage();
    ...
    public void onTransTimeout(TransactionClient transaction) {
        if (transaction.getTransactionMethod().equals(SipMethods.REGISTER)) {
            ...
            if (loop) {
                ...
                if (inter_time_msecs > SipStack.regc_max_attempt_timeout)
                    inter_time_msecs = SipStack.regc_max_attempt_timeout;
