...
public class NotImplementedServer implements SipProviderListener {
     ...
     */
    public void onReceivedMessage(SipProvider sip_provider, Message msg) {  // respond
        if (msg.isRequest() && !msg.isAck() && !msg.isCancel()) {
            ...
            if (implemented_methods != null) {
                for (int i = 0; i < implemented_methods.length; i++)
                    if (method.equalsIgnoreCase(implemented_methods[i])) is_implemented = true;
            }
            if (!is_implemented) {
