...
public abstract class BaseMessageFactory {
     ...
     * @see #createRequest(String, SipURL, NameAddress, NameAddress, NameAddress, String, String, int, String, long, String, String, String, String)
     */
    public static Message createRequest(SipProvider sip_provider, String method, SipURL request_uri, NameAddress to, NameAddress from, NameAddress contact, String body) {  //SipURL request_uri=to.getAddress();
    ...
    public static Message createRequest(Dialog dialog, String method, String body) {
        ...
        if (contact == null)
            contact = new NameAddress(new SipURL(sip_provider.getViaAddress(), sip_provider.getPort()));
   ...
   {  return createRequest(SipMethods.ACK,dialog,body);
    ...
    public static Message createCancelRequest(Message req) {
        ...
        if (req.hasAuthorizationHeader())
            cancel.setAuthorizationHeader(req.getAuthorizationHeader());
        if (req.hasProxyAuthorizationHeader())
            cancel.setProxyAuthorizationHeader(req.getProxyAuthorizationHeader());
        return cancel;
    }
