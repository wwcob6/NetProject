...
*/
    ...
    public boolean startsWithIgnoreCase(String[] ss) {
        for (int i = 0; i < ss.length; i++) {
            ...
            for (int k = 0; k < ss[i].length() && (index + k) < str.length(); k++) {
                if (!(equal = (compareIgnoreCase(ss[i].charAt(k), str.charAt(index + k)) == 0)))
                    break;
            }
   ...
   {  return skipWSP();
    ...
    public Vector getIntVector(char[] separators) {
        ...
        do {
            list.addElement(Integer.valueOf(getWord(separators)));
        } while (hasMore());
        return list;
    }
    ...
    public Vector getIntVector() {
        Vector list = new Vector();
        do {
            list.addElement(Integer.valueOf(getString()));
        } while (hasMore());
        return list;
    }
