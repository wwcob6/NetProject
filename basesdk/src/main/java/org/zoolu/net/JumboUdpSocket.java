...
public class JumboUdpSocket extends UdpSocket {
    ...
    public void receive(UdpPacket pkt) throws java.io.IOException {
        ...
        if (!jumbo_packet.isFragment()) {
            ...
            if (JumboPacket.isComplete(fragments)) {  // last fragment
                try {
                    ...
                    if (DEBUG)
                        println("Received: " + jp.toString() + " (" + fragments.size() + " FRAGS)");
                } catch (Exception e) {
                    e.printStackTrace();
                    System.exit(0);
                }
    ...
    public void send(UdpPacket pkt) throws java.io.IOException {
        ...
        if (DEBUG)
            println("Send: " + jumbo_packet.toString() + " (" + fragments.length + " FRAGS)");
        ...
        if (table_ids.containsKey(t)) {
            ...
            if (JumboUdpSocket.DEBUG) {
                ...
                for (int i = 0; i < frags.size(); i++)
                    total_len += ((JumboPacket) frags.elementAt(i)).getDataLength();
