...
public class TlsSocketFactory {
    ...
    private void initSupportedProtocols() {
        try {
            ...
            if (supported_protocols == null)
                supported_protocols = ssl_socket.getSupportedProtocols();
     ...
     */
    public TlsSocket createTlsSocket(String host, int port) throws IOException {  //SSLSocket ssl_socket=(SSLSocket)ssl_factory.createSocket(host,port);
     ...
     * Creates a new TlsSocket
     */
    public TlsSocket createTlsSocket(IpAddress ipaddr, int port) throws IOException {  //SSLSocket ssl_socket=(SSLSocket)ssl_factory.createSocket(ipaddr.getInetAddress(),port);
        //return new TlsSocket(ssl_socket);
        SSLSocket ssl_socket = (SSLSocket) ssl_factory.createSocket();
        if (!client_mode) ssl_socket.setUseClientMode(false);
        if (enabled_protocols != null) ssl_socket.setEnabledProtocols(enabled_protocols);
