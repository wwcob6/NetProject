   ...
   {  v=new SdpField('v',"0");
    ...
    public SessionDescriptor(SessionDescriptor sd) {
        ...
        for (int i = 0; i < sd.av.size(); i++)
            av.addElement(new AttributeField((AttributeField) sd.av.elementAt(i)));
        for (int i = 0; i < sd.media.size(); i++)
            media.addElement(new MediaDescriptor((MediaDescriptor) sd.media.elementAt(i)));
    }
    ...
    public SessionDescriptor(String sdp) {
        ...
        while (par.hasMore() && (!par.startsWith("a=") && !par.startsWith("m="))) {  // skip unknown lines..
            par.goToNextLine();
        }
     ...
     */
    public OriginField getOrigin() {  //System.out.println("DEBUG: inside SessionDescriptor.getOwner(): sdp=\n"+toString());
        return o;
    }
     ...
     * @return this SessionDescriptor
     */
    public SessionDescriptor addMedia(MediaField media, AttributeField attribute) {  //printlog("DEBUG: media: "+media,5);
     ...
     * @return this SessionDescriptor
     */
    public SessionDescriptor addMedia(MediaField media, Vector attributes) {  //printlog("DEBUG: media: "+media,5);
     ...
     * @return this SessionDescriptor
     */
    public SessionDescriptor addMediaDescriptor(MediaDescriptor media_desc) {  //printlog("DEBUG: media desc: "+media_desc,5);
     ...
     * @return this SessionDescriptor
     */
    public SessionDescriptor addMediaDescriptors(Vector media_descs) {  //media.addAll(media_descs); // not supported by J2ME..
    ...
    public SessionDescriptor removeMediaDescriptor(String media_type) {
        for (int i = media.size() - 1; i >= 0; i--)
            if (((MediaDescriptor) media.elementAt(i)).getMedia().getMedia().equals(media_type))
     ...
     * Removes all MediaDescriptors
     */
    public SessionDescriptor removeMediaDescriptors() {  //media.clear(); // not supported by J2ME..
        media.setSize(0);
        return this;
    }
    ...
    public SessionDescriptor addAttributes(Vector attributes) {
        for (int i = 0; i < attributes.size(); i++)
            addAttribute((AttributeField) attributes.elementAt(i));
        return this;
    }
    ...
    public boolean hasAttribute(String attribute_name) {
        for (int i = 0; i < av.size(); i++) {
            if (((AttributeField) av.elementAt(i)).getAttributeName().equals(attribute_name))
                return true;
        }
        return false;
    }
     ...
     */
    public String toString() {  //String str=v.toString()+o.toString()+s.toString();
